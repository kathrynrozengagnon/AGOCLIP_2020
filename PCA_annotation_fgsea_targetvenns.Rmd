...
## unfiltered PCA of all samples (Supplementary Figure 5d)
...
input gtf file is available for download at [Vectorbase](https://vectorbase.org/vectorbase/app/downloads/)
```{r }
...
##Test filtering conditions, Supplementary Figure 5a-c
...
##fgsea of targeted genes/pathways that drive Ago1 and Ago seperation by PCA; Figures 3d,e,g,h and Supplementary Tables 5 and 6
...
testAag2 <- sdvcAag2 %>% dplyr::group_by(leadingEdge) %>% dplyr::top_n(1, NES) %>% dplyr::summarise(Term = paste0(Term, collapse=","),pval=dplyr::first(pval),size= dplyr::first(size), NES =dplyr::first(NES)) #this is Supplementary Table 5
...
testaegypti <- sdvcaegypti %>% group_by(leadingEdge) %>% top_n(1, NES) %>% dplyr::summarise(Term = paste0(Term, collapse=","),pval=dplyr::first(pval),size=dplyr::first(size), NES =dplyr::first(NES)) #this is Supplementary Table 6

```

##Make annotation pie charts on final filtered peaks, Figure 3i-l
...
##Now do Venn Diagrams to compare genes in different filtered ab/lysate combinations, Supplementary Figure 5e
...
## Venn Diagram to compare genes found in Zhang et al. and my aegypti Ago1 genes, Supplementary Figure 5f
