...
## Annotate and filter for PCA calculations - this table is the basis for Table S3, and was used to test different filtering parameters for Figure S2A-C
...
v <- merge(all, allannodf, by="peakID", all.x=TRUE)
all <- v[,c(1:200, 206:214)]
...
length(which(all$Aag2Ago1_peakBC > 0))
length(which(all$Aag2Ago1_peakBC > 0 & grepl("3' UTR", all$annotation)))
length(which(all$Aag2Ago1_peakBC > 0))
length(which(all$Aag2Ago1_peakBC > 0 & grepl("3' UTR", all$annotation)))
...
##fgsea of targeted genes/pathways that drive Ago1 and Ago seperation by PCA; Figures 3F, 3I
```{r}
...
##Make annotation pie charts on filtered peaks, Figure 3A-D
```{r}
...
```{r}
Zhang <- read.delim("/Users/kathryn/Reprocess_all_paper_datasets/Zhang_peaks.txt", header = TRUE, sep = "\t") #this is Dataset S03 from Zhang et al., 2017
