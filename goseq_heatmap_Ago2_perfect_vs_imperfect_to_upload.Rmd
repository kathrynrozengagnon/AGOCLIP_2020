...
genome fasta is available at [Vectorbase](https://vectorbase.org/vectorbase/app/downloads/); input gtf file used to be available for download on Vectorbase as well, but I am unsure if the gtf file is still available there; it is too large to upload to Github, but is available upon request
...
##get gene lists
all_with_pats <- read.delim("/Users/kathryn/Reprocess_all_paper_datasets/Supp_tables/Table_S4_Resource_paper.txt", header = TRUE, sep="\t")
...
##Goseq, Table S7
output/input files "GOsem_up.txt" was uploaded to [REVIGO](http://revigo.irb.hr/) to simplify by semantic redundancy, generating "REVIGO_FINAL.csv", a simplified list of biological processes; both files are available in my [Github](https://github.com/kathrynrozengagnon/AGOCLIP_2020) 
```{r }
