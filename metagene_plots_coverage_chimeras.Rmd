...
```{r}
#plot coverage over 3UTR; have provided Rdata in my github so the user can start from here
...
# pattern <-  "/Users/kathryn/Reprocess_all_paper_datasets/seed_search_refseqs/all_unique_filt_seeds.fa"                        
# motifSeq <- readDNAStringSet(pattern)
# pattern <- as.character(motifSeq)
# 
# loop_test <- vector("list", length(pattern[1:4]))
# for(i in 1:length(pattern[1:4])){
#   message("Search for ",DNAString(pattern[i]))
#   loop_test[i] <- vmatchPattern(pattern=DNAString(pattern[i]),subject=genome) %>% unlist()}
# names(loop_test) <- names(pattern[1:4])
...
```{r}
##plot chimera and predicted 6mer matches over 3UTR; have provided Rdata in my github so the user can start from here
