...
## Prepare files for miRDeep2, which takes uncollapsed short fastqs normally; processing steps are commented out
...
##Align uncollapsed files split by sample to genome using bowtie2

```{r}
#genome fasta used to make index available at: https://www.vectorbase.org/downloads
...
##concatenate uncollapsed, processed reads by sample type (Ab/lysate) and map to genome using bowtie2
...
##Run also using standard miRDeep2 method where all files are put together to run miRDeep2.pl, and a config.txt file is provided to assign ids; mapping was done by miRDeep2 method (Bowtie v1.2.2)
...
##Get mirdeep results from each method and filter novel small RNAs discovered

```{r}
#individual samples, bowtie2
...
#samples concatenated by Ab/lysate, bowtie2
...
##Now quantify all that met miRDeep2 filtering critera by reverse mapping small RNA reference sequences to short reads (18-30nt) with linker artifacts removed; mapping steps are shown below but commented out
```{r}
...
##Filter novel and known small RNAs by abundance criteria in my libraries and graph  Figure  3L and K

```{r}
#Tables available in my github
...
##Get length distribution by type for Figures S3A and S3B
...
##Classify known and novel RNAs detected in Aag2 cells compared to whole mosquitoes, Figures S3C and S3D
